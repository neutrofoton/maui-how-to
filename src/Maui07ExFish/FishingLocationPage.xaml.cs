...
namespace Maui07ExFish
{
    public partial class FishingLocationPage : ContentPage
    {
        ...
        public FishingLocationPage()
        {
            InitializeComponent();

            this.viewModel = CreateDummyPanoramaViewModel();
            this.BindingContext = this.viewModel;
        }


        private PanoramaViewModel CreateDummyPanoramaViewModel()
        {

            MainViewModel mainViewModel = new MainViewModel();
            FishingLocationViewModel locationViewModel = mainViewModel.FishingLocations.FirstOrDefault();

            PanoramaViewModel panViewModel = new PanoramaViewModel(locationViewModel)
            {
                WeatherIcon = mainViewModel.WeatherIcon
            };
        ...
        private void PanGestureRecognizer_PanUpdated(object sender, PanUpdatedEventArgs e)
        {
            switch (e.StatusType)
            {
                ...
                case GestureStatus.Running:
                    ...
                    var xOffset = e.TotalX * DeviceDisplay.MainDisplayInfo.Density;
        ...
        private void Panorama_Touch(object sender, SKTouchEventArgs e)
        {
            switch (e.ActionType)
            {
        ...
        private void BiteTimeChart_PaintSurface(object sender, SKPaintSurfaceEventArgs e)
        {
            var info = e.Info;
            var surface = e.Surface;
